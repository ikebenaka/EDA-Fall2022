...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #'expression' adds text to the plot and you specify its position before.
print(Plot_text)
...
PeterPaul.chem.nutrients.surf <- PeterPaul.chem.nutrients %>%
  filter(depth == 0)
# Color your points by year, and facet by lake in two rows.
# Change the ylab name
ggplot(PeterPaul.chem.nutrients.surf) + 
...
ggplot(PeterPaul.chem.nutrients) + 
  ...
  scale_color_viridis(option = "magma", direction = -1) + #-1 direction makes more sense because warmer temperatures are represented by brighter colors.
  ylab("Temperature (C)") + #specify y axis title
  xlab("") + #we dont want an x axis title 
  scale_x_date(date_breaks = "5 years",date_labels = "%Y") #specify how large the breaks should be and specify that the labels should list years
...
ggplot(EPAair)+
  geom_violin(aes(x = as.factor(Year), y = PM2.5))
