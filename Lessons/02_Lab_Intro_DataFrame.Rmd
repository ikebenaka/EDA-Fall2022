...
create_df <- data.frame("speed"=car_speed,"dist"=car_dist) #creates a dataframe with column names with values from specified variables
create_df
```
...
nrow(df)  #number of variables(columns)
...
df_2 <- df[1:10,1:2] #df[rows,columns]

#you could also use this notation to delete columns
df <- df[,-2] #can chose to not specify any rows and it will just act on columns insteaddf
df
```