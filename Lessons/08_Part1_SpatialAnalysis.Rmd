...
sfg_GraingerHall <- st_point(c(-78.9427,36.00465)) #creates a simple feature point
class(sfg_GraingerHall)
mapview(sfg_GraingerHall)

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #locates the point on Earth
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #transform = re-project
...
#try to make plot where EPA site points are larger based on their distance from gas plants.
