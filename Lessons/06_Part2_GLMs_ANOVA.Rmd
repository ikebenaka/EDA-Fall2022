...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_040"]) #For each one, looking at dry mass and filtering by the site name
...
Litter.Totals.groups #Letters indicate which groups are considered to have the same mean. If two groups contain different letters, then they have a significantly different mean.
...
# If we wanted to present this information in a report, we would want all the similar groups next to each other in the chart.
```
...
summary(Litter.anova.2way) #the p value here indicates that our means are different, but does not tell us which ones are different.

# Format as lm
Litter.anova.2way2 <- lm(data = Litter, dryMass ~ functionalGroup + nlcdClass)
summary(Litter.anova.2way2) #This shows p-value for each functional group litter type and land cover. 

# Run a post-hoc test for pairwise differences
TukeyHSD(Litter.anova.2way) #This gets us even more information
...
summary(Litter.anova.2way3) #Gives significance of functional group interaction with land cover class
