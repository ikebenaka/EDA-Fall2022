...
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # instead of listing numbers by each column, they are added row by row
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "quatro", "cinco"), 
                                  c("un", "deux", "trois", "quatre"))) # dimnames specifies  names for rows and columns



matrix1[4, ] # Everything on row 4
matrix1[ , 3] # Everything in column 3
matrix1[c(12, 14)] # Lists the 12th and 14th vector in the matrix table
matrix1[c(12:14)] # Lists the vectors between 12th and 14th vectors.
matrix1[2:4, 1:3] # 
...
list1
list1[[2]] # can also use $ to pull a certain item from list

# Data Frames ----
d <- c(1, 2, 3, 4) # What type of vector?
e <- c("red", "white", "red", NA) # What type of vector?
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector?
dataframe1 <- data.frame(d,e,f) # creates dataframe
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # adds names to the columns
...
```{r DataExercise-1a}
highs_F <- c(93,73,78,85,82,78,75,73,71,76)
lows_F <- c(53,48,59,61,54,51,51,52,55,56)
```
...
```{r DataExercise-1b}
FtoC <- function(highs_F) {(highs_F - 32) * (5 / 9)}
highs_C <- FtoC(highs_F)
lows_C <- FtoC(lows_F)
```
...
```{r DataExercise-1c}
forecast <- data.frame(highs_F,highs_C,lows_F,lows_C)
names(forecast) <- c("Highs (F)","Highs (C)","Lows (F)","Lows (C)")
forecast
```
...
date_obj_today <- ymd(str_today) # reads the string and converts it to a YEAR-Month-Day format
date_obj_today
