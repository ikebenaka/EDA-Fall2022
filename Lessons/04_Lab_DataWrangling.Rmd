...
> Add notes here: To see how species are impacted and affect these values.
...
# setting strings as factors establishes categories for character-based strings
...
NTL.phys.data.Tuesday90s <- filter(NTL.phys.data.Tuesday, year4 %in% c(1990:1999))
# OR
...
NTL.phys.data.Tuesday90s.July <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>% 
  filter(daynum %in% c(182:212))
# OR
NTL.phys.data.Tuesday90s.July2 <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>% 
  mutate(month = month(sampledate)) %>% #This creates a new column in the table that has the month number for each entry.
  filter(month == 7) #This filters to only get entries in July.
...
SurfaceTemp <- 
  ...
  summarize(mean_ST = mean(temperature_C))
