...
#Exercise 1:
#1.
O3Densityplot <- ggplot(EPAair, aes(x=Ozone, fill = as.factor(Year)))+
  geom_density(alpha=.5)+
  scale_fill_manual(values=c("darkgreen", "darkblue"))+
  labs(x="Ozone AQI Value", y="Density", fill="")
plot(O3Densityplot)
#2.
O3Boxplot <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone))+
  geom_boxplot()+
...
plot(O3Boxplot)
#3.
O3Violinplot <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone))+
  geom_violin()+
  stat_summary(geom = "text", fun = max, vjust=-1, size=4,
               label=c("b","a"))+
  labs(x="","Ozone AQI value")+
...
regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(regression)
# p-value=2.2e^-16 and R2=0.1314
...
PMbyO3 <- ggplot(EPAair, aes(x=Ozone,y=PM2.5))+
  geom_point()+
  geom_smooth(method = "lm", se=FALSE)+
  geom_text(x = 100, y = 90, label = eq(EPAair$Ozone,EPAair$PM2.5), parse = TRUE)
...
EPAaircorr <- cor(EPAairsubset)
corrplot(EPAaircorr, method = "ellipse")
corrplot.mixed(EPAaircorr, supper = "ellipse")
...
ozonebyall <- lm(data=EPAairsubset, PM2.5 ~ Ozone + Year + Month + meanlong+ meanlat)
...
ozonebest-regression <- lm(data=EPAairsubset, PM2.5 ~ Ozone + Month + meanlong + meanlat)
```
