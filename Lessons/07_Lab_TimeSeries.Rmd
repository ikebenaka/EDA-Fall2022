...
#It looks like temperatures are trending upwards, but there is no discernible trend for rainfall over time from the plot.
...
Rainfall_ts_decomp <- stl(Rainfall_ts, s.window="periodic")
...
Temp_components <- as.data.frame(Temp_ts_decomp$time.series[,1:3])
Temp_components <- mutate(Temp_components, Observed=Temp_BR_processed$Temperature_C, Date=Temp_BR_processed$Date)

Rainfall_components <- as.data.frame(Rainfall_ts_decomp$time.series[,1:3])
Rainfall_components <- mutate(Rainfall_components, Observed=Rainfall_BR_processed$Rainfall, Date=Rainfall_BR_processed$Date)
...
Temp_trend <- Kendall::SeasonalMannKendall(Temp_ts)
Temp_trend
summary(Temp_trend)

Rainfall_trend <- Kendall::SeasonalMannKendall(Rainfall_ts)
Rainfall_trend
...
Temp_trend <- Kendall:MannKendall(Temp_ts)
