...
gage_data <- read.table(theURL2,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 
colnames(gage_data) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
gage_data <- gage_data %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
...
```{r}
#Apply the API key
census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22", install = TRUE, overwrite = TRUE)
...
elec.df <- data.frame(States = the_states,
         Price = the_price,
...
#1 Link to the web site using read_html
the_website <- read_html("https://www.eia.gov/electricity/state/archive/2019/")

#2&3 Locate elements and read their text attributes into variables
the_states2019 <- the_website %>% 
  html_nodes("td:nth-child(1)") %>% 
  html_text()

the_price2019 <- the_website %>% 
  html_nodes("td:nth-child(2)") %>% 
  html_text()

the_capacity2019 <- the_website %>% 
  html_nodes("td:nth-child(3)") %>% 
  html_text()

net_generation2019 <- the_website %>% 
  html_nodes("td:nth-child(4)") %>% 
  html_text()

total_retail2019 <- the_website %>% 
  html_nodes("td:nth-child(5)") %>% 
  html_text()
  
#3 Construct a dataframe from the values
elec.df2019 <- data.frame(States = the_states2019,
         Price = the_price2019,
         Capacity = the_capacity2019,
         Net.Generation = net_generation2019,
         Total.Retail = total_retail2019)
```
...
the_df <- data.frame(States = the_states2019,
         Price = the_price2019,
         Capacity = the_capacity2019,
         Net.Generation = net_generation2019,
         Total.Retail = total_retail2019) %>% mutate(Year=the_year)
...
df.2017 <- scrape.it(2017)

# Map the function to scrape data from 2017 to 2019
df.17.19 <- map(seq(2017,2019),scrape.it) %>% bind_rows() #bind rows puts the three indiv dataframes together
