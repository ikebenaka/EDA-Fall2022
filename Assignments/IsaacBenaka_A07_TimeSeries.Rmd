...
GaringerOzone.daily.ts <- ts(GaringerOzone.clean$Ozone.clean,  start=c(f_year,f_month,f_day), frequency=365)
...
```{r}
#12
adf.test(GaringerOzone.monthly.ts)
Kendall::SeasonalMannKendall(GaringerOzone.daily.ts)

```

> Answer: The adf.test function tests whether a trend is stationary. Given that the p-value of the test is <0.05, we accept the alternative hypotesis that the trend is stationary. Because the trend exhibits constant variance over time as the result of seasonality, it is appropriate to run a Seasonal Mann-Kendall test.
...
> Answer: The data shows no change in ozone concentrations through the 2010s at this station.
	(Augmented Dickey-Fuller Test data:  GaringerOzone.monthly.ts Dickey-Fuller = -8.6413, Lag order = 4, p-value = 0.01 alternative hypothesis: stationary)
...
> Answer: By bemoving the seasonal component from the time series data, our p-value decreased significantly. Because the p-value became even smaller than 0.05, the removal of the seasonal component accentuated the trend present in the data.
